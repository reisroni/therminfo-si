...
/**********************************
* selba.php
* Controlador da pagina 'structural elba'
* Criado: 19-08-2011
* Modificado: 02-02-2014
* Copyright (c) 2014, ThermInfo 
***********************************/

class Selba extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$smiles = $this->input->post('smiles'); // SMILES para previsao
			$smiles_chiral = $this->input->post('smiles_chiral'); // SMILES para previsao
			$mol_file = $this->input->post('molfile'); // Mol file para previsao
			$ebonds = $this->input->post('sebonds_select'); // Ligacoes duplas (trans)
            $file_name = date('d-m-Y_His');
			$newsmiles = $this->obabel->molfile_to_smiles($mol_file, $file_name);
			
			if ($newsmiles)
			{
				// * Efectua e formata a previsao
				$this->data['result'] = $this->_selba_smiles($newsmiles, $ebonds);
			}
			else
			{
			...
			$this->load->view('content/selba_view', $this->data);
		}
		else
		{
			// Formulario nao submetido (pagina 'Structural ELBA')
			$this->load->view('content/selba_view', $this->data);
		}
	}
	...
	public function pred_db()
	{
			...
			if ($props)
			{
				...
				if (count($props) > 1)
				{
					...
					if (isset($mid) && $mid)
					{
						...
						$gas_exp = 'n.a.';
						$vap_exp = 'n.a.';
						$query_result = $this->Data_value_model->find_ent_values($mid);
		...
		{
			// Redirecionar para a pagina 'Structural ELBA'
			redirect('selba');
		}
	}
	...
	public function pred_nodb()
	{
		...
		{
			// Redireciona para a pagina 'Structural ELBA'
			redirect('selba');
		}
	}
	
	/*
	 * Previsao estrutural do composto
	 * 
	 ...
	 */
	private function _selba_smiles($smiles, $bonds)
	{
        $html_result;
		...
		{
			// Campo em branco
			$this->data['info'] = '<p class="errorPane"><strong>You have not entered molecule details. Please try again!</strong></p>';
		}
		else
		{
			// Pesquisa na DB
			...
			{
				// Erro na BD
				$error_code = 'TI_601'; // SElba: 6, selba_smiles: 0, DB Error: 1
				$error_date = date('Y-m-d');
				log_message('error', "[SElba] {$error_date} - [{$error_code}]: An error occurred in the database.");
			...
			else
			{
				...
				// Botao para previsao
				$this->data['info'] .= "<p><form action='". base_url('selba/pred_nodb') ."' method='post' target='_blank'>
				...
				{
					// Outra situacao
					$error_code = 'TI_602'; // SElba: 6, selba_smiles: 0, Other Error: 2
                    $error_date = date('Y-m-d');
                    log_message('error', "[SElba] {$error_date} - [{$error_code}]: An error occurred.");
	...
	private function _html_result($data, $bonds, $smiles) 
	{
		...
		for ($i = 0; $i < $limit; ++$i)
		{
			...
			$html .= "<td><form action='". base_url('selba/pred_db') ."' method='post' target='_blank'>
...
/* End of file selba.php */
/* Location: ./application/controllers/selba.php */